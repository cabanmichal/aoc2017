...
class StreamProcessor(object):
    def __init__(self, astring=None):
    ...
    def from_file(cls, filename):
        with open(filename, 'r', encoding='utf-8') as fh:
            ...
            return __class__(fh.read().strip())

    @staticmethod
    def process_escapes(astring):
        string_builder = []
        ...
        while i < string_length:
            if astring[i] == '!':
                ...
                string_builder.append(astring[i])
                i += 1

        return ''.join(string_builder)

    @staticmethod
    def remove_garbage(astring):
        string_builder = []
        ...
        while i < string_length:
            if in_garbage:
                while astring[i] != '>' and i < string_length:
                    i += 1
                    garbage_count += 1
        ...
        return ''.join(string_builder), garbage_count

    @staticmethod
    def score_groups(astring):
        score = 0
        debth = 0
    ...
    def get_score(self):
        if self.astring is not None:
            ...
            clean_string, _ = __class__.remove_garbage(__class__.process_escapes(astring))

            return __class__.score_groups(clean_string)

    def count_garbage(self):
        if self.astring is not None:
            astring = self.astring
            _, garbage_count = __class__.remove_garbage(__class__.process_escapes(astring))
...
if __name__ == '__main__':
    ...
    """
    for s in ['<{!>}>', '<!!>', '<!!!>>', '<{o"i!a,<{i<a>']:
        print(process_escapes(s))

    for s in ['{<{},{},{{}}>}', '{<a>,<a>,<a>,<a>}', '{{<a>},{<a>},{<a>},{<a>}}', '{{<!>},{<!>},{<!>},{<a>}}']:
        print(remove_garbage(process_escapes(s)))

    for s in ['{}', '{{{}}}', '{{},{}}', '{{{},{},{{}}}}', '{<a>,<a>,<a>,<a>}', '{{<ab>},{<ab>},{<ab>},{<ab>}}',
              '{{<!!>},{<!!>},{<!!>},{<!!>}}', '{{<a!>},{<a!>},{<a!>},{<ab>}}']:
        print(score_groups(remove_garbage(process_escapes(s))))
    """

    stream_processor = StreamProcessor().from_file('day09_input.txt')
    print(stream_processor.get_score())  # 14190
    print(stream_processor.count_garbage())  # 7053
