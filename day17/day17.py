...
def solve_part_one():
    ...
    for value in range(1, 2017 + 1):
        current_position = (current_position + PUZZLE_INPUT) % buffer_size + 1
...
def solve_part_two():
    ...
    for value in range(2, 50000000 + 1):
        current_position = (current_position + PUZZLE_INPUT) % buffer_size + 1
        if current_position == 1:
            buffer_1_value = value

        buffer_size += 1

    print(buffer_1_value)


if __name__ == '__main__':
    solve_part_one()  # 1561
    solve_part_two()  # 33454823
