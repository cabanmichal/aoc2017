...
For example, if the previous example was your first round, you would start your second round with the same length
...
use numeric bitwise XOR to combine each consecutive block of 16 numbers in the sparse hash (there are 16 such blocks
in a list of 256 numbers). So, the first element in the dense hash is the first sixteen elements of the sparse hash
XOR'd together, the second element in the dense hash is the second sixteen elements of the sparse hash XOR'd together,
etc.
...
the final output is the dense hash in hexadecimal notation. Because each number in your dense hash will be between
0 and 255 (inclusive), always represent each number as two hexadecimal digits (including a leading zero as necessary).
So, if your first three numbers are 64, 7, 255, they correspond to the hexadecimal numbers 40, 07, ff, and so
the first six characters of the hash would be 4007ff. Because every Knot Hash is sixteen such numbers, the hexadecimal
...
class KnotHasher(object):
    def __init__(self, lengths, alist=None):
        if alist is None:
            self._original_list = list(range(256))
        else:
            self._original_list = [e for e in alist]
        self._length_suffix = [17, 31, 73, 47, 23]
        self._repetitions = 64
        self._chunk_size = 16

        if isinstance(lengths, str):
            self._lengths = [ord(c) for c in lengths]
            self._lengths.extend(self._length_suffix)
        else:
            self._lengths = [length for length in lengths]

        self._current_hasher_state = {'current_idx': 0,
                                      'skip_size': 0}

        self._hashed_list = [e for e in self._original_list]

        self.dense_hash = ''
        self.product_of_first_two = None

        self._init()

    def _init(self):
        self._hash_list()
        self.product_of_first_two = self._hashed_list[0] * self._hashed_list[1]
        self._make_dense_hash()

    def _hash_list(self):
        list_length = len(self._hashed_list)

        for length in self._lengths:
            if length > list_length:
                continue

            start = self._current_hasher_state['current_idx']
            end = (start + length) % list_length
            skip_size = self._current_hasher_state['skip_size']

            for i in range(length // 2):
                s = (start + i) % list_length
                e = end - 1 - i
                self._hashed_list[s], self._hashed_list[e] = self._hashed_list[e], self._hashed_list[s]

            start += (length + skip_size)
            start %= list_length
            skip_size += 1

            self._current_hasher_state['current_idx'] = start
            self._current_hasher_state['skip_size'] = skip_size

    def _make_sparse_hash(self):
        for _ in range(self._repetitions - 1):
            self._hash_list()

    def _make_dense_hash(self):
        self._make_sparse_hash()

        dense_hash = []
        for i in range(0, len(self._hashed_list), self._chunk_size):
            dense_hash.append(__class__.xor_list(self._hashed_list[i:i+self._chunk_size]))

        self.dense_hash = ''.join("{:02x}".format(v) for v in dense_hash)
    ...
    def get_product_of_first_two(self):
        return self.product_of_first_two

    def get_dense_hash(self):
        return self.dense_hash

    def get_original_list(self):
        return [e for e in self._original_list]


if __name__ == '__main__':
    hasher = KnotHasher((3, 4, 1, 5), list(range(5)))
    print(hasher.get_product_of_first_two())  # 12

    hasher = KnotHasher(map(int, PUZZLE_INPUT.split(',')))
    print(hasher.get_product_of_first_two())  # 19591

    hasher = KnotHasher(PUZZLE_INPUT)
    print(hasher.get_dense_hash())  # 62e2204d2ca4f4924f6e7a80f1288786
